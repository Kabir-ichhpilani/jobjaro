    ...
    return (
        <div className="bg-purple-900 md:inline-block hidden text-white w-[60px] rounded-md md:min-w-[230px] min-h-screen flex flex-col fixed md:static z-50">
            ...
            <div className="flex flex-col items-center md:items-start space-y-1 px-6 py-8">
                <SidebarItem icon={a1} label="Dashboard" link="/" currentPath={location.pathname} />
                ...
                <SidebarItem icon={a7} label="Insights" link="/insights" currentPath={location.pathname} />
                <SidebarItem icon={a8} label="Bulk CV Upload" link="/resume-parser" currentPath={location.pathname} />
                <SidebarItem icon={a9} label="Team Access" link="/team-members" currentPath={location.pathname} />
                <SidebarItem icon={a10} label="Chat" link="/chat" currentPath={location.pathname} />
            </div>
...
const SidebarItem = ({ icon, label, link, currentPath }) => {
    const isActive = currentPath === link;

    return (
        <Link to={link}>
            <div
                className={`flex items-center justify-center md:justify-start w-[200px] px-2 py-2 text-sm ${
